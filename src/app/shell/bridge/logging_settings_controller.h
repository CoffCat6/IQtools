...
class LoggingSettingsController : public QObject {
    Q_OBJECT
    Q_PROPERTY(bool consoleEnabled READ consoleEnabled WRITE setConsoleEnabled NOTIFY settingsChanged)
    Q_PROPERTY(bool fileEnabled READ fileEnabled WRITE setFileEnabled NOTIFY settingsChanged)
    Q_PROPERTY(QString logDirectory READ logDirectory WRITE setLogDirectory NOTIFY settingsChanged)
    Q_PROPERTY(QString minimumLevel READ minimumLevel WRITE setMinimumLevel NOTIFY settingsChanged)
...
public:
    explicit LoggingSettingsController(QObject* parent = nullptr);
...
private:
    ...
    static iqtools::infra::logging::LogLevel levelFromString(const QString& levelText);
