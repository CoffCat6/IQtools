...
LoggingSettingsController::LoggingSettingsController(
    iqtools::core::SettingsManager* settingsManager, QObject *parent)
    : QObject(parent), m_settingsManager(settingsManager) {
  // Load from persisted settings first
  loadFromSettings();
  
  // Get current runtime config as baseline
  m_applied = iqtools::infra::logging::Logger::config();
  
  // Override with persisted settings
  m_applied.enableConsoleOutput = m_settingsManager->logConsoleEnabled();
  m_applied.enableFileOutput = m_settingsManager->logFileEnabled();
  m_applied.minimumLevel = levelFromString(m_settingsManager->logMinimumLevel());
  
  QString savedDir = m_settingsManager->logDirectory();
  if (!savedDir.isEmpty()) {
    m_applied.logDirectory = savedDir;
  } else if (m_applied.logDirectory.isEmpty()) {
    m_applied.logDirectory = defaultLogDirectory();
  }
  
  m_pending = m_applied;
  
  // Apply the loaded settings to the logger
  iqtools::infra::logging::Logger::applyConfig(m_applied);
}

void LoggingSettingsController::loadFromSettings() {
  // This is called during construction to sync with persisted settings
}
...
bool LoggingSettingsController::apply() {
  if (m_pending.enableFileOutput &&
      !iqtools::infra::logging::Logger::isLogDirectoryWritable(
          m_pending.logDirectory)) {
  ...
  if (m_settingsManager != nullptr) {
    m_settingsManager->setLogConsoleEnabled(m_applied.enableConsoleOutput);
    m_settingsManager->setLogFileEnabled(m_applied.enableFileOutput);
    m_settingsManager->setLogMinimumLevel(levelToString(m_applied.minimumLevel));
    m_settingsManager->setLogDirectory(m_applied.logDirectory);
  }
  
  m_statusMessage = QStringLiteral("日志设置已应用并保存");
  emit settingsChanged();
  emit statusMessageChanged();
...
QString LoggingSettingsController::levelToString(
    iqtools::infra::logging::LogLevel level) {
  switch (level) {
...
iqtools::infra::logging::LogLevel
LoggingSettingsController::levelFromString(const QString &levelText) {
